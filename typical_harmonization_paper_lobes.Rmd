...
```{r}
ggplot(data_lobes, aes(x = Age, y = AvgThickness, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
  facet_grid(ROI ~.)
...
```{r}
ggplot(data_lobes, aes(x = Age, y = K, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
  facet_grid(ROI ~.)
...
# m.1 <- lme4::lmer(AvgThickness ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(AvgThickness_shiftc = AvgThickness - T_shift)
```

### TotalArea \~ Age

```{r eval=FALSE, include=FALSE}
m.1 <- lme4::lmer(TotalArea ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(TotalArea_shiftc = TotalArea - AT_shift)
```

### ExposedArea \~ Age

```{r eval=FALSE, include=FALSE}
m.1 <- lme4::lmer(ExposedArea ~ Age * Diagnostic * ROI + (1|Sample) + (1|Sample:Diagnostic) + (1|Sample:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(ExposedArea_shiftc = ExposedArea - AE_shift)
```
...
# m.1 <- lme4::lmer(localGI ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# m.1 <- lme4::lmer(K ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(K_shiftc = K - K_shift)
```
...
# m.1 <- lme4::lmer(S ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(S_shiftc = S - S_shift)
```
...
# m.1 <- lme4::lmer(I ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# data_lobes <- full_join(data_lobes, re) %>% mutate(I_shiftc = I - I_shift)
```
...
# After harmonization
```{r}
ggplot(data_lobes, aes(x = Age, y = AvgThickness_shiftc, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
  facet_grid(ROI ~.)
...
```{r}
ggplot(data_lobes, aes(x = Age, y = K_shiftc, color = Sample)) + 
  geom_point() + 
  theme_pubr() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)) + 
  facet_grid(ROI ~.)
...
```{r}
amostra_Coef <- filter(data_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]") %>%
    group_by(ROI, Age_interval) %>%
...
amostras_Coef_age <- filter(data_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]") %>%
    group_by(Age_interval) %>%
    summarise(
        N = n_distinct(SUBJ),
        min_age = min(Age),
...
ggplot(filter(data_lobes, ROI != "hemisphere", Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]"), aes(y = K_shiftc, x = Age_interval), aes(y = K, x = Age_interval)) +
    geom_boxplot() + 
    theme_pubr() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)
...
# m.1 <- lme4::lmer(AvgThickness_shiftc ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# m.1 <- lme4::lmer(localGI_shiftc ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# m.1 <- lme4::lmer(K_shiftc ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# m.1 <- lme4::lmer(S_shiftc ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
...
# m.1 <- lme4::lmer(I_shiftc ~ Age * Diagnostic * ROI + (1|Sample:Diagnostic:ROI), data = data_lobes)
