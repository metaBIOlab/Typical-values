...
```{r echo=FALSE}
ggplot(filter(data, ROI != "hemisphere"), aes(x = ROI, y = c, color = Sample)) +
  geom_boxplot() +
  geom_jitter() +
  theme_pubr()
```
...
ggplot(filter(data_lobes, Diagnostic == "CTL", Age_interval != "[0,5)", Age_interval != "[95,100]"), aes(y = K, x = Age_interval)) +
    geom_boxplot() + 
    theme_pubr() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=9)) +
  facet_grid(ROI ~ .)
